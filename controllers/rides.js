  ...
  client.get(url, function(data, response) {
    ...
    for (var i = 0; i < 1; i++) {
      ...
      for (var j = 0; j < 1; j++) {
        ...
        var distanceValue = Number(element.distance.value);
        var distance = element.distance.text;
        var duration = element.duration.text;
        computeFare(req, res, source_address, destination_address, distance, distanceValue, duration);
      }
    }
  });
}

function computeFare(req, res, src, dest, distance, distanceValue, duration) {
  var pricePerFifthKm = 3.0;
  var fare = (distanceValue * pricePerFifthKm * 5) / 1000;
  var response = {
    'source_address': src,
